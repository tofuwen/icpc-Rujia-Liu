...
double DistanceToLine(Point P, Point A, Point B) {
  ...
  return fabs(Cross(v1, v2)) / Length(v1); // Èç¹û²»È¡¾ø¶ÔÖµ£¬µÃµ½µÄÊÇÓÐÏò¾àÀë
}
...
int getLineCircleIntersection(Line L, Circle C, double& t1, double& t2, vector<Point>& sol){
  ...
  double delta = f*f - 4*e*g; // ÅÐ±ðÊ½
  if(dcmp(delta) < 0) return 0; // ÏàÀë
  if(dcmp(delta) == 0) { // ÏàÇÐ
...
int getCircleCircleIntersection(Circle C1, Circle C2, vector<Point>& sol) {
  ...
  if(dcmp(d) == 0) {
    if(dcmp(C1.r - C2.r) == 0) return -1; // ÖØºÏ£¬ÎÞÇî¶à½»µã
    return 0;
  }
...
// ¹ýµãpµ½Ô²CµÄÇÐÏß¡£v[i]ÊÇµÚiÌõÇÐÏßµÄÏòÁ¿¡£·µ»ØÇÐÏßÌõÊý
int getTangents(Point p, Circle C, Vector* v) {
  Vector u = C.c - p;
  double dist = Length(u);
  if(dist < C.r) return 0;
  else if(dcmp(dist - C.r) == 0) { // pÔÚÔ²ÉÏ£¬Ö»ÓÐÒ»ÌõÇÐÏß
    v[0] = Rotate(u, PI/2);
    return 1;
  } else {
