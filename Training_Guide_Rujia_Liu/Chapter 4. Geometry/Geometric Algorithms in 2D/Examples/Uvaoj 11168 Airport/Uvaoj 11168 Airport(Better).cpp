...
double solve(Point a, Point b, double x, double y, int n){
	double A, B, C;
	if(dcmp(a.x-b.x)==0){
		B = 0;
		A = 1;
	...
	C = -A*a.x-B*a.y;
	return fabs((A*x+B*y+n*C)/sqrt(A*A+B*B));
}
	...
	for(int cc=1;cc<=T;cc++){
		...
		if(n>1)
			for(int i=0;i<num;i++){
				// double ans = 0;
				// for(int j=0;j<n;j++)
				// 	ans += DistanceToLine(p[j], ch[i], ch[i+1]);
				// ans /= n;
				// minimum = min(ans,minimum);
				double ans = solve(ch[i],ch[i+1], allx, ally,n);
				ans /= n;
				minimum = min(ans,minimum);
			}
		else
