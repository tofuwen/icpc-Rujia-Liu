...
int main()
{
	...
	for(int cc=0;cc<T;cc++){
		...
		for(int i=0;i<n;i++){
			...
			phi = phi*pi/180;
			//P.S. 这两种写法都是对的。
			 pp[0] = Point(x + h*sin(phi)/2 + w*cos(phi)/2, y + h*cos(phi)/2 - w*sin(phi)/2);
			 pp[1] = Point(x + h*sin(phi)/2 - w*cos(phi)/2, y + h*cos(phi)/2 + w*sin(phi)/2);
			// double cita = atan(h/w);
			// double angle1 = cita-phi;
			// double angle2 = pi-cita-phi;
			// double r = sqrt(w*w+h*h)/2;
			// pp[0] = Point(x + cos(angle1)*r, y + sin(angle1)*r);
			// pp[1] = Point(x + cos(angle2)*r, y + sin(angle2)*r);
			pp[2] = start*2 - pp[0];
			pp[3] = start*2 - pp[1];
			for(int j=0;j<4;j++){
				p[num++] = pp[j];
		...
		int numC = ConvexHull(p, num, ch);
		double S = fabs(PolygonArea(ch, numC));
		printf("%.1f %%\n",100*area/S);
	}
	return 0;
}
